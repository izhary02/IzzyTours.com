...
async function updatePartialLocation(location:LocationModel):Promise<LocationModel> {
    // const errors = location.validatePatch();
    // if(errors){
    //     throw new ValidationError(errors);
    // }
...
const TakeTheName = imageInfo.shift()
const imageName = TakeTheName.imageName
const StringImageName = JSON.stringify(imageInfo)
console.log(`imageName ${imageInfo}`);
console.log(`successfully deleted`+StringImageName);
console.log(StringImageName);
